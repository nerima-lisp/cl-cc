...
- **対象**: `packages/vm/src/vm.lisp`, `packages/vm/src/vm-run.lisp`, `src/jit/baseline.lisp`
...
- **対象**: `src/jit/`, `packages/vm/src/vm.lisp`
...
- **対象**: `packages/optimize/src/optimizer.lisp`, 新規`packages/optimize/src/ipa.lisp`
...
- **内容**: 型推論（`packages/type/src/inference.lisp`）の結果でレシーバー型が単態の場合、`vm-generic-call`を`vm-call`に変換（確実なdevirt）。型が確率的に単態の場合は**投機的devirt**（PICガード+deoptバックエッジ）
...
- **対象**: 新規`packages/optimize/src/alias.lisp`, `packages/optimize/src/optimizer.lisp`
...
- **対象**: 新規`packages/optimize/src/points-to.lisp`
...
- **対象**: `packages/type/src/inference.lisp`, `packages/optimize/src/effects.lisp`
...
- **対象**: 新規`packages/compile/src/simd-builtins.lisp`, `packages/compile/src/builtin-registry.lisp`
- **現状**: `builtin-registry.lisp`の~210エントリにSIMD演算なし
- **内容**: `(cl-cc:simd:f32x4-add a b)` 形式の明示的SIMDイントリンシック。`f32x4`, `f64x2`, `i32x8`等のSIMDベクタ型。`packages/compile/src/simd-builtins.lisp`でbuiltinエントリとして登録。CL配列↔SIMDベクタ変換ヘルパ
...
- **対象**: `packages/compile/src/codegen.lisp`, テストフレームワーク
...
- **対象**: `packages/vm/src/vm.lisp`, `packages/pipeline/pipeline.lisp`, `packages/cli/src/main.lisp`
...
- **対象**: `packages/emit/src/aarch64-codegen.lisp`, `packages/runtime/src/heap.lisp`
...
- **対象**: `packages/emit/src/x86-64-codegen.lisp`, `packages/binary/src/macho.lisp`
