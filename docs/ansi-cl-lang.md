...
ANSI CL では `define-method-combination` の長形式ボディ内で使う特殊なエラー関数。cl-cc は `macros-clos-protocol.lisp` で長形式 DSL をサポートし、`call-method`/`make-method` を `stdlib-source-clos.lisp` で提供する。

> 注: `method-qualifiers` は method descriptor (`:qualifiers` キー) から取得可能。primary method は nil、before/after/around は各 qualifier を返す。
...
cl-cc の `define-method-combination` は長形式 DSL を `macros-clos-protocol.lisp` でサポートする。`call-method`、`make-method` は `stdlib-source-clos.lisp` で提供。method descriptor (`vm-clos-execute.lisp`) が `:qualifiers` を保持し、`method-qualifiers` がアクセス可能。

```lisp
;; SBCL / ANSI での長形式例 (cl-cc で対応)
...
| 要素                       | cl-cc | SBCL | 備考                                                                        |
| -------------------------- | ----- | ---- | --------------------------------------------------------------------------- |
| method group specification | ✅    | ✅   | `macros-clos-protocol.lisp` の `%method-comb-register-long` で処理          |
| `call-method`              | ✅    | ✅   | `stdlib-source-clos.lisp` で実装; method descriptor から `:function` を抽出 |
| `make-method`              | ✅    | ✅   | `stdlib-source-clos.lisp` で実装; `eval` による合成メソッド生成             |

**SBCL での長形式完全例 (cl-cc で対応):**
...
| 項目                                                      | マージ規則 | cl-cc | SBCL                                                              |
| --------------------------------------------------------- | ---------- | ----- | ----------------------------------------------------------------- |
| `compute-effective-slot-definition` 相当の専用 MOP フック | ✅         | ✅    | 備考: F1 — VM命令へコンパイルされる専用effective-slotマージモデル |
| 継承後スロットを `class-slots` から観測できる             | ✅         | ✅    |
| 継承後スロットに対する `slot-value` / accessor 利用       | ✅         | ✅    |
| `:initarg` 和集合を保証する専用実装根拠                   | ✅         | ✅    | 備考: F2 — 継承initargマージ、most-specific-initform優先          |
| `:initform` 優先順位を公開 MOP として制御                 | ✅         | ✅    | 備考: F2 — most-specific-classのinitformを採用                    |
...
| ユーザー定義メタクラス上書き                | ✅    | ✅   | 備考: F3 — slot-value-using-class setf, slot-boundp/makunbound-using-class, shared-initialize |
...
| multiple dispatch memoization     | ✅    | ✅   | 備考: F4 — tuple-key inline cacheでmulti-dispatch GF呼び出しを高速化     |
| hidden class / shape ベース最適化 | ✅    | —    | 備考: F5 — O(1) slot indexによるvector-backed instance向け形状メタデータ |
...
| SBCL の `standard-instance` | ✅    | ✅   | 備考: F6 — arefベースのvector-backed instanceレイアウト, unbound-slot-marker |

### 13.3 静的最適化

| 最適化                                 | cl-cc | SBCL | 備考                                                                            |
| -------------------------------------- | ----- | ---- | ------------------------------------------------------------------------------- |
| sealed / final 想定の静的ディスパッチ  | ✅    | ✅   | 備考: F7b — optimizerでsealed+satiated single-method GFをdevirtualize           |
| SBCL `deftransform` (メソッド特化変換) | ✅    | ✅   | 備考: F10 — _deftransform-table_, define-deftransform, compiler-macroexpand連携 |
...
| 段階                                       | cl-cc | SBCL | 説明                                                                                               |
| ------------------------------------------ | ----- | ---- | -------------------------------------------------------------------------------------------------- |
| `slot-value-using-class` の公開 MOP フック | ✅    | ✅   | `stdlib-source-clos.lisp` で `defgeneric`+`defmethod` 実装; `(setf slot-value-using-class)` も対応 |
...
cl-cc では `find-method` / `add-method` / `remove-method` の操作が可能であり、`standard-method` / `standard-reader-method` / `standard-writer-method` / `standard-accessor-method` の MOP クラス階層を `stdlib-source-clos.lisp` で提供する。

| 機能                                                           | cl-cc | SBCL | 備考                                                                                                                                |
| -------------------------------------------------------------- | ----- | ---- | ----------------------------------------------------------------------------------------------------------------------------------- |
| メソッドオブジェクトの追加・削除                               | ✅    | ✅   | `add-method` / `remove-method` は利用可能                                                                                           |
| メソッド検索                                                   | ✅    | ✅   | `find-method` を提供                                                                                                                |
| `method-qualifiers` 等の公開 MOP accessor                      | ✅    | ✅   | `stdlib-source-clos.lisp` で実装; primary method は NIL 返却; qualified method 対応は method metaobject 化後に拡張                  |
| `standard-method` / `standard-reader-method` などの MOP クラス | ✅    | ✅   | `stdlib-source-clos.lisp` で `defclass` 定義; `standard-reader-method` / `standard-writer-method` / `standard-accessor-method` 含む |
...
| 機能                       | cl-cc | SBCL | 備考                                                                           |
| -------------------------- | ----- | ---- | ------------------------------------------------------------------------------ |
| `sb-mop`                   | ✅    | ✅   | 備考: F9 — 標準MOPシンボルエクスポート                                         |
| sealed classes             | ✅    | ✅   | 備考: F7a — :**sealed**フラグ, subclass/redefinition拒否, define-sealed-type   |
| satiated generic functions | ✅    | ✅   | 備考: F8 — :**satiated**フラグ, permanent IC cache, register-methodでunsatiate |
| `closer-mop`               | ✅    | ✅   | 備考: F9 — 標準MOPシンボルエクスポート、互換層ライブラリ同等機能を組み込み     |
...
| 機能                                | cl-cc | SBCL | 備考                                                           |
| ----------------------------------- | ----- | ---- | -------------------------------------------------------------- |
| `sb-ext:define-sealed-type`         | ✅    | ✅   | 備考: F7a — 継承・再定義の自由度と引き換えに最適化余地を増やす |
| sealed class 前提の静的ディスパッチ | ✅    | ✅   | 備考: F7b — コンパイラがメソッド候補集合を閉じて扱える         |
...
| 機能                                     | cl-cc | SBCL | 備考                                                |
| ---------------------------------------- | ----- | ---- | --------------------------------------------------- |
| satiated generic function 最適化         | ✅    | ✅   | 備考: F8 — hot path の dispatch cost を下げる最適化 |
| `sb-pcl::satiating-gfs-p` などの内部 API | ✅    | ✅   | 備考: F12 — satiated状態判定の互換プレディケート    |
...
| CLOS インスタンスの `copy` (非標準)          | ✅    | —    | 備考: F11 — vector/hash-table dual-layout対応のcopy-instance |
...
- Metaclass protocol hooks (F3: `slot-value-using-class` setf, `slot-boundp`/`slot-makunbound-using-class`, `shared-initialize`)
- Multiple dispatch memoization (F4: tuple-key IC for multi-dispatch GF calls)
- Shape metadata / `slot-definition-location` (F5: O(1) slot index for vector-backed instances)
- Vector-backed `standard-instance` (F6: aref-based instance layout, `unbound-slot-marker`)
- Sealed classes (F7a: `:__sealed__` flag, subclass/redefinition rejection, `define-sealed-type`)
- Static dispatch optimization (F7b: devirtualize sealed+satiated single-method GFs in optimizer)
- Satiated generic functions (F8: `:__satiated__` flag, permanent IC cache, `register-method` unsatiates)
- `sb-mop` / `closer-mop` packages (F9: standard MOP symbol exports)
- Deftransform infrastructure (F10: `*deftransform-table*`, `define-deftransform`, compiler-macroexpand integration)
- `copy-instance` hardening (F11: vector/hash-table dual-layout support)
- `sb-pcl::satiating-gfs-p` predicate (F12: compatibility predicate)

**Prior Note**: 2026-05-19 — Implemented: CLI `--timeout`, `slot-value-using-class` (reader+setf), `method-qualifiers`, `standard-method`/`standard-reader-method`/`standard-accessor-method`/`standard-writer-method` MOP classes, long-form `define-method-combination` with `call-method`/`make-method`, method descriptor infrastructure (`vm-register-method` stores `:function`/`:qualifiers` in descriptors).
