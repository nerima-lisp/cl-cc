...
(defparameter *wasm-validate-timeout-seconds* 30
  "Timeout in seconds for external Wasm validation tools (node, wasmtime).
On timeout validation is reported as failed rather than hanging the CLI.")
  ...
  (let ((ok t))
    (when (cl-cc/codegen:wasm-tool-available-p "node")
      (let ((script (format nil "const fs=require('fs');process.exit(WebAssembly.validate(fs.readFileSync(~S))?0:1)" path)))
        (handler-case
            (sb-ext:with-timeout *wasm-validate-timeout-seconds*
              (uiop:run-program (list "node" "-e" script) :ignore-error-status nil))
          (sb-ext:timeout ()
            (setf ok nil)
            (format *error-output* "WebAssembly.validate timed out after ~A second~:P~%"
                    *wasm-validate-timeout-seconds*))
          (error (e)
            (setf ok nil)
    ...
    (when (cl-cc/codegen:wasm-tool-available-p "wasmtime")
      (handler-case
        ...
        (sb-ext:timeout ()
          (setf ok nil)
          (format *error-output* "wasmtime validate timed out after ~A second~:P~%"
                  *wasm-validate-timeout-seconds*))
        (error (e)
          (setf ok nil)
