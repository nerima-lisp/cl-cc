...
(deftest-each parser-lambda-list-parsing
  "parse-compiler-lambda-list: required-only, &whole, &environment, &optional, &rest, &key, &aux, and empty lists."
  :cases (("required-only"
           (lambda ()
...
(deftest-each parser-lambda-list-has-extended-p
  ...
  :cases (("whole"    '(&whole w x)   t)
          ("environment" '(&environment e x) t)
          ("optional" '(x &optional y) t)
          ("rest"     '(x &rest args)  t)
          ("key"      '(x &key y)      t)
          ("aux"      '(x &aux y)      t)
          ("simple"   '(x y z)         nil)
