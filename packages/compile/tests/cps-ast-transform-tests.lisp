...
(it-sequential "fr-373-bootstrap-sexp-cps-if uses Common Lisp truthiness" (dolist (case (list (list nil 22) (list 0 11) (list 0.0 11) (list (complex 0 0) 11) (list :non-nil 11))) (destructuring-bind (condition expected) case (let* ((input (list (quote if) condition (quote (print 11)) (quote (print 22)))) (transformed (cl-cc:cps-transform* input)) (result nil) (output (with-output-to-string (*standard-output*) (setf result (funcall (eval transformed) (function identity)))))) (expect result :to-equal expected) (expect output :to-equal (format nil "~%~S " expected))))))
