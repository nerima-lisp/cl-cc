...
(it-sequential "remove-if-macro-no-stdlib remove-if-empty"
  (destructuring-bind (expected form) (list nil "(remove-if #'minusp '(-1 -2))")
    (expect (run-string form) :to-equal expected)))
...
(it-sequential "unwind-protect-cleanup-runs-on-the-error-path"
  (expect (= 5 (run-string
               "(let ((x 0))
...
(it-sequential "unwind-protect-cleanup-runs-on-the-normal-path"
  (expect (run-string "(let ((x nil)) (unwind-protect 1 (setq x t)) x)") :to-be t))
...
(it-sequential "copy-seq-sequences symbols"
  (destructuring-bind (expected form) (list '(cl-cc::a cl-cc::b cl-cc::c) "(copy-seq '(a b c))")
    (expect (run-string form) :to-equal expected)))
...
(it-sequential "copy-seq-nested-wrapper-prints"
  (expect (prin1-to-string (run-string "(list (copy-seq (vector 1 2)) 9)")) :to-equal "(#(1 2) 9)"))
