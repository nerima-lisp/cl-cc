...
(deftest-each cps-single-param-lambda-p
  ...
  :cases (("yes-single"     t   '(lambda (x) x))
          ...
          ("no-atom"        nil 42))
  (expected form)
  (assert-equal expected (cl-cc/cps::%single-param-lambda-p form)))
...
(deftest-each cps-funcall-of-single-lambda-p
  ...
  :cases (("yes"            t   '(funcall (lambda (x) x) 42))
          ...
          ("no-atom"        nil 42))
  (expected form)
  (assert-equal expected (cl-cc/cps::%funcall-of-single-lambda-p form)))
...
(deftest-each cps-eta-reducible-lambda-p
  ...
  (expected form)
  (assert-equal expected (cl-cc/cps::%eta-reducible-lambda-p form)))
...
(deftest-each cps-simplify-form-reductions
  ...
  (expected form)
  (assert-equal expected (cl-cc/cps::cps-simplify-form form)))
