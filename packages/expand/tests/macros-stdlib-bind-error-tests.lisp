...
(it-sequential "assert-expansion"
  (let* ((basic  (our-macroexpand-1 '(assert test)))
    ...
    (expect 'let :to-be (car (caddr basic)))
    (expect (%find-call-in-form 'cerror (caddr basic)) :to-be-truthy)
    (expect (%find-call-in-form 'signal (caddr basic)) :to-be-truthy)
    (expect (%find-call-in-form 'cerror (caddr custom)) :to-be-truthy)
    (expect (second (first (second (caddr custom)))) :to-equal "bad input")))
