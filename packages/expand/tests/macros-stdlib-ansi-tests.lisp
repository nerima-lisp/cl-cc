...
(it-sequential "assert-failure-body-contains-cerror"
  (let ((result (our-macroexpand-1 '(assert (zerop n)))))
    (expect (%find-call-in-form 'cerror (cddr result)) :to-be-truthy)))

(it-sequential "assert-failure-body-signals-before-cerror"
  (let ((result (our-macroexpand-1 '(assert (zerop n)))))
    (expect (%find-call-in-form 'signal (cddr result)) :to-be-truthy)))
...
(it-sequential "assert-datum-forwarded-to-cerror"
  (let* ((result (our-macroexpand-1 '(assert test nil "msg ~A" x)))
    ...
    (expect format-call :to-be-truthy)
    (expect (third format-call) :to-equal "msg ~A")))
