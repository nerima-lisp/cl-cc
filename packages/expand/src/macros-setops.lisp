...
(defun %sequence-result-form (result-kind acc-form)
  "Return the result form for indexed sequence operations."
  (case result-kind
    (vector `(coerce ,acc-form 'vector))
    (string `(coerce ,acc-form 'string))
    (otherwise acc-form)))
                ...
                (if hash-compatible-p
                    `(if (> (length ,seq) ,*set-hash-threshold*)
                        ...
                        (let ((,acc nil)
                              ...
                              (,kfn-var ,(or key '#'identity)))
                          (dotimes (,i (length ,seq)
                                       ,(%sequence-result-form result-kind `(nreverse ,acc)))
                            (let ((,x ,ref-form))
                              (unless (find (funcall ,kfn-var ,x) ,acc :test ,tst-var :key ,kfn-var)
                                (setq ,acc (cons ,x ,acc)))))))
                    `(let ((,acc nil)
  ...
  (let ((l1 (gensym "L1")) (l2 (gensym "L2")) (x (gensym "X"))
    ...
    (let ((slow-form
      ...
      (%set-hash-fast-path-expand
       list1 list2 test test-not slow-form
       (lambda (l1 l2 x acc seen kx kfn-var hash-test)
         `(let ((,acc nil)
                (,seen (make-hash-table :test ,hash-test))
                (,kfn-var ,(or key '#'identity)))
            (dolist (,x ,l2)
...
(defun %set-hash-fast-path-expand (list1 list2 test test-not slow-form fast-form-builder)
  "Return a shared hash-table fast-path wrapper for binary set operations."
  (multiple-value-bind (hash-test hash-compatible-p)
      (%hash-test-designator-form test test-not)
    (let ((l1 (gensym "L1")) (l2 (gensym "L2")) (x (gensym "X"))
          (acc (gensym "ACC")) (seen (gensym "SEEN")) (kx (gensym "KEYED-X"))
      ...
      `(let ((,l1 ,list1) (,l2 ,list2))
         ,(if hash-compatible-p
              `(if (> (+ (length ,l1) (length ,l2)) ,*set-hash-threshold*)
                   ,(funcall fast-form-builder l1 l2 x acc seen kx kfn-var hash-test)
                   ,slow-form)
              slow-form)))))
...
(defun %set-difference-expand (list1 list2 test key test-not)
  ...
  (let ((slow-form (%set-filter-expand list1 list2 'unless
...
(defun %intersection-expand (list1 list2 test key test-not)
  ...
  (let ((slow-form (%set-filter-expand list1 list2 'when
                                       (%keyword-test-key-args test test-not key))))
    (%set-hash-fast-path-expand
     list1 list2 test test-not slow-form
     (lambda (l1 l2 x acc seen kx kfn-var hash-test)
