...
(it-sequential "io-file-position-set-returns-boolean"
  (let ((s (io-vm-full)))
    (cl-cc/vm::vm-reg-set s :contents "abcdef")
    (io-exec (cl-cc:make-vm-make-string-stream :dst :stream :direction :input
                                                :initial-string :contents) s)
    (flet ((seek (position)
             (cl-cc/vm::vm-reg-set s :new-position position)
             (io-exec (cl-cc:make-vm-file-position :dst :ok :handle :stream
                                                    :position :new-position) s)
             (cl-cc/vm::vm-reg-get s :ok))
           (stream-position ()
             (io-exec (cl-cc:make-vm-file-position :dst :position :handle :stream) s)
             (cl-cc/vm::vm-reg-get s :position)))
      (expect (stream-position) :to-equal 0)
      (expect (seek 3) :to-equal t)
      (io-exec (cl-cc:make-vm-read-char :dst :character :handle :stream) s)
      (expect (cl-cc/vm::vm-reg-get s :character) :to-equal #\d)
      (expect (seek :current) :to-equal t)
      (expect (stream-position) :to-equal 4)
      (expect (seek :start) :to-equal t)
      (io-exec (cl-cc:make-vm-read-char :dst :character :handle :stream) s)
      (expect (cl-cc/vm::vm-reg-get s :character) :to-equal #\a)
      (expect (seek :end) :to-equal t)
      (expect (stream-position) :to-equal 6))))

(it-sequential "io-file-length-binary-stream"
  (let ((path (merge-pathnames (format nil "clcc-io-file-length-~A.bin" (gensym))
                               (uiop:temporary-directory)))
        (large-position (+ (ash 1 31) 17))
        (s (io-vm-full)))
    (unwind-protect
         (with-open-file (stream path :direction :io :element-type '(unsigned-byte 8)
           ...
           (flet ((seek (position)
                    ...
                    (cl-cc/vm::vm-reg-get s :ok))
                  (stream-position ()
                    (io-exec (cl-cc:make-vm-file-position :dst :position
                                                          :handle :stream) s)
                    (cl-cc/vm::vm-reg-get s :position)))
             (expect (seek :start) :to-equal t)
             (expect (stream-position) :to-equal 0)
             (expect (seek :end) :to-equal t)
             (expect (stream-position) :to-equal 3)
             (expect (seek :current) :to-equal t)
             (expect (stream-position) :to-equal 3)
             (expect (seek large-position) :to-equal t)
             (write-byte 255 stream)
             (finish-output stream)
             (expect (stream-position) :to-equal (1+ large-position))
             (io-exec (cl-cc:make-vm-file-length :dst :length :handle :stream) s)
             (expect (cl-cc/vm::vm-reg-get s :length) :to-equal (1+ large-position))
             (expect (seek large-position) :to-equal t)
             (expect (read-byte stream) :to-equal 255)))
