...
(it-sequential "integer-interval-abstract-interpretation-propagates-const-and-add"
  (let* ((cfg (cl-cc/optimize:cfg-build
         ...
         (out (gethash entry (cl-cc/optimize:opt-dataflow-result-out result))))
    (expect (cl-cc/optimize::opt-interval-lo (gethash :r0 out)) :to-equal 10)
    (expect (cl-cc/optimize::opt-interval-hi (gethash :r0 out)) :to-equal 10)
    (expect (cl-cc/optimize::opt-interval-lo (gethash :r1 out)) :to-equal 20)
    (expect (cl-cc/optimize::opt-interval-hi (gethash :r1 out)) :to-equal 20)
    (expect (cl-cc/optimize::opt-interval-lo (gethash :r2 out)) :to-equal 30)
    (expect (cl-cc/optimize::opt-interval-hi (gethash :r2 out)) :to-equal 30)))
