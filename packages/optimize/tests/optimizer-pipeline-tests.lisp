...
(defun %assert-opt-default-convergence-edge (before after)
  (expect (< (%opt-default-convergence-key-position before)
                  (%opt-default-convergence-key-position after)) :to-be-truthy))
...
(defun %assert-opt-ic-patch-plan-case (site current-state next-state target expected-kind)
  (expect (cl-cc/optimize::opt-ic-patch-patch-kind
              (cl-cc/optimize::opt-ic-make-patch-plan
               site current-state next-state target)) :to-be expected-kind))
...
(defun %assert-opt-atomic-opcode-case (target operation memory-order expected-opcode)
  (expect (cl-cc/optimize::opt-select-atomic-opcode
              :target target
              :operation operation
              :memory-order memory-order) :to-be expected-opcode))
...
(defun %opt-assert-program-changed (optimized program)
  (expect (equal (%opt-instruction-sexps optimized)
                       (%opt-instruction-sexps program)) :to-be-falsy))
