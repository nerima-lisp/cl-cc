...
Returns true when any test failed or any expected-fail test unexpectedly passed.
xfail = expected failures that still fail (ok).  xpass = expected-fail tests
that now unexpectedly pass (gate failure — the fix may have landed)."
  (let* ((pass-count (%count-results-by-status flat-results :pass))
         ...
         (xfail-count (%count-results-by-status flat-results :xfail))
         (xpass-count (%count-results-by-status flat-results :xpass))
         (total      (length flat-results))
         (any-fail   (or (> fail-count 0) (> xpass-count 0))))
    (if (%verbose-tap-p)
         (let ((bar "#  ---------------------------------------------------"))
           ...
           (when (> xfail-count 0)
             (format t "#   XFAIL  ~4D  (expected failures, ok)~%" xfail-count))
           (when (> xpass-count 0)
             (format t "#   XPASS  ~4D  (unexpected pass — gate FAILURE)~%" xpass-count))
           ...
           (when (> fail-count 0)
             ...
             (format t "~A~%" bar))
           (when (> xpass-count 0)
             (format t "#~%#  Unexpected passes (XPASS — fix landed?):~%")
             (dolist (r (sort (remove-if-not (lambda (r) (eq (getf r :status) :xpass)) flat-results)
                              #'< :key (lambda (r) (getf r :number))))
               (format t "#    [~4D] ~A~%" (getf r :number) (getf r :name)))
             (format t "~A~%" bar))
           (format t "#~%"))
         (let ((skip-str (if (> skip-count 0) (format nil "  ~A skipped" skip-count) ""))
               (xfail-str (if (> xfail-count 0) (format nil "  ~A xfail" xfail-count) ""))
               (xpass-str (if (> xpass-count 0) (format nil "  ~A xpass (GATE FAILURE)" xpass-count) "")))
           (format t "# ~A passed  ~A failed~A~A~A~%"
                   pass-count fail-count skip-str xfail-str xpass-str)))
    any-fail))
